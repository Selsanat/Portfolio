...
namespace NorthLab.Effects
{
    public class RetroScreen : MonoBehaviour
    {
        ...
        [SerializeField, Space, Tooltip("Sync transforms of the DisplayCamera with the RenderCamera")]
        private bool syncTransform = true;
        [SerializeField, Tooltip("Template object for the DisplayCamera so you could attach components(like screen filters). If you link the prefab from the assets RetroScreen will instantiate it on the initialization.")]
        private GameObject templateDisplayObject = null;
        [SerializeField, Space, Tooltip("Render texture filtering mode. Use Point for that retro look.")]
        ...
        private void Init()
        {
            ...
            if (templateDisplayObject)
            {
                if (!templateDisplayObject.scene.IsValid())
                {
                    templateDisplayObject = Instantiate(templateDisplayObject, Vector3.zero, Quaternion.identity);
                }

                Camera camComp = templateDisplayObject.GetComponent<Camera>();
                if (camComp)
                    DisplayCamera = camComp;
                else
                    DisplayCamera = templateDisplayObject.AddComponent<Camera>();
            }
            else
            {
